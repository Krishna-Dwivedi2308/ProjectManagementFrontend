...
const PasswordResetDialog = ({ open, onOpenChange }) => {
    ...
    const handleSend = async (e) => {
        ...
        setstate({
            loading: true, error: '', success: false, message: ''
        });
        try {
            ...
            setstate((prev) => ({ ...prev, loading: false, success: true, message: "Mail has been sent to your email. Please check your inbox." }));
        }
        catch (err) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Forgot your password? No problem, we’ve got you <div className=""></div></DialogTitle>
                    <DialogDescription>
                ...
                <form onSubmit={handleSend} className="space-y-3">
                    <div>
                        <Input
                            ...
                            aria-label="Email"
                        />
                        {state?.validationError?.email && <p className="text-red-500 text-sm mt-1">{state?.validationError?.email}</p>}
                    </div>
                    ...
                    <DialogFooter>
                        <div className="flex gap-2">
                            <Button type="button" variant="ghost" onClick={() => onOpenChange(false)} disabled={state.loading}>
                                Cancel
                            </Button>
